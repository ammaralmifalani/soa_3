...
class OrderTest {
    ...
    void testCalculatePrice_StudentOnWeekday() {
        ...
        order.addTicket(new MovieTicket(screening, false)); // Ticket 2 (gratis)
        order.addTicket(new MovieTicket(screening, true));  // Ticket 3 (premium)
        order.addTicket(new MovieTicket(screening, true));  // Ticket 4 (gratis)

        double expectedPrice = 10.0 + 0.0 + (10.0 + 2.0) + 0.0;
    ...
    void testCalculatePrice_StudentWeekend() {
        ...
        order.addTicket(new MovieTicket(screening, false)); // Ticket 1
        order.addTicket(new MovieTicket(screening, true));  // Ticket 2 (gratis)
        order.addTicket(new MovieTicket(screening, true));  // Ticket 3 (premium)
        order.addTicket(new MovieTicket(screening, false)); // Ticket 4 (gratis)

        double expectedPrice = 12.0 + 0.0 + (12.0 + 2.0) + 0.0;
        assertEquals(expectedPrice, order.calculatePrice(), 0.01);
    }
    ...
    void testCalculatePrice_NonStudentWeekday() {
        ...
        double expectedPrice = 15.0 + 0.0 + (15.0 + 3.0) + 0.0;
        assertEquals(expectedPrice, order.calculatePrice(), 0.01);
    }
